...
class Agent:
    ...
    def _experiment_value(self, c, n, incumbent_net, contact_opportunity):
        """Expected improvement of the conservative final forecast, less pilot cost.

        Under the existing normal model the next posterior mean has variance
        prior_variance - posterior_variance. Integrate its positive surplus over
        the best tested alternative for the SAME audience. This is a planning
        heuristic, not an estimate of the evaluator's incremental final score.
        """
        new_var = 1 / (1 / c["variance"] + n / 0.804**2)
        spread = c["arpu"] * math.sqrt(max(0, c["variance"] - new_var))
        surplus = self._net(c, 0) - c["arpu"] * math.sqrt(new_var) - incumbent_net
        if spread > 0:
            z = surplus / spread
            expected = spread * math.exp(-z*z/2) / math.sqrt(2*math.pi)
            expected += surplus * (1 + math.erf(z / math.sqrt(2))) / 2
        else:
            expected = max(surplus, 0)
        current = max(self._net(c) - incumbent_net, 0)
        return expected - current - n * (c["cost"] + contact_opportunity)
    ...
    def act(self, env) -> list[dict]:
        ...
        for step in range(16):
            if env.pilots_left <= 0:
                self._event("pilot_stop", reason="environment_pilot_limit")
                break
            if time.monotonic() - start > 480:
                self._event("pilot_stop", reason="timeout")
                break
            choices = []
            exclusions = {"search_space": 0, "resources": 0, "no_estimated_value": 0}
            tested = [c for c in candidates if c["n"] > 0]
            # A conservative opportunity cost of consuming a finite contact.
            best_known = max([0.] + [self._net(c) for c in tested
                                    if c["size"] <= env.remaining_contacts and
                                    c["size"] * c["cost"] <= env.remaining_budget])
            contact_opportunity = best_known / max(env.remaining_contacts, 1)
            for i, c in enumerate(candidates):
                if c["failed"] or len(c["pilots"]) >= 3 or c["size"] < 10:
                    exclusions["search_space"] += 1
                    continue
                if c["size"] > env.remaining_contacts or c["size"] * c["cost"] > env.remaining_budget:
                    exclusions["resources"] += 1
                    continue
                # Six is an initial diversity target, never a ban on new arms.
                if successful >= 6 and c["n"] > 0 and abs(
                        c["mean"] - c["cost"] * c["size"] / max(c["arpu"], 1)
                ) > 2.5 * math.sqrt(c["variance"]):
                    exclusions["no_estimated_value"] += 1
                    continue
                ...
                if successful >= 6:
                    incumbent = max([0.] + [self._net(other) for other in tested
                                           if other is not c and other["cell"] == c["cell"]])
                    value = self._experiment_value(c, n, incumbent, contact_opportunity)
                    if value <= 0:
                        exclusions["no_estimated_value"] += 1
                        continue
                diversity = int(successful < 6 and c["cell"] not in tested_cells)
                utility = value if successful >= 6 else max(optimistic, 0) + max(value, 0)
                priority = utility / (1 + c["size"] * c["cost"] / max(env.remaining_budget, 1))
                choices.append((diversity, priority, int(c["cell"] not in tested_cells), -i, n, value))
            if not choices:
                reason = ("no_estimated_value" if exclusions["no_estimated_value"] else
                          "insufficient_resources" if exclusions["resources"] else "search_space_exhausted")
                self._event("pilot_stop", reason=reason, exclusions=exclusions)
                break
            _, _, _, minus_i, n, value = max(choices)
            i = -minus_i
            c = candidates[i]
            self._event("pilot_choice", index=i, new_hypothesis=c["n"] == 0,
                        successful_before=successful, estimated_value=value,
                        reason="initial_diversity" if successful < 6 else "value_of_information")
            try:
            ...
            self._event("pilot_stop", reason="environment_pilot_limit" if env.pilots_left <= 0 else "internal_pilot_limit")
        budget, contacts = float(env.remaining_budget), int(env.remaining_contacts)
        selected, used = [], set()
        # Recompute scarcity-aware standalone forecast after each allocation.
        ...
        while len(selected) < 10:
            ...
            if not positive:
                ...
                reason = "positive risk-adjusted standalone forecast; disjoint final audiences; fits resources"
            ...
            self._event("selected", index=i, campaign=selected[-1], size=chosen["size"],
                        ...
                        forecast_scope="final_standalone_excluding_pilot_overlap",
                        reason=reason)
