"""Stage1-only behavior checks, plus an explicit restored-main limitation.

The six exploration checks exercise the frozen experimental source, not agent.py.
No organizer effect model is used and no seed tuning is performed here.
"""
import unittest
...
from agent import Agent as MainAgent, audience
from split_eval import ROOT, load_agent, sha256


EXPERIMENT_SOURCE = ROOT / "experiments" / "stage1_expanded_search" / "agent.py"
EXPERIMENT_SHA256 = "43b88055a4f71af19b39ff49694b124b11873cd812ff9a0b9388e57867279658"
...
class Stage1ExperimentalExplorationTests(unittest.TestCase):
    """All six original stage1 behavior tests run only on its preserved source."""

    @classmethod
    def setUpClass(cls):
        if sha256(EXPERIMENT_SOURCE) != EXPERIMENT_SHA256:
            raise AssertionError("Frozen stage1 agent.py changed; test scope is no longer valid")
        # exec-based loading never creates __pycache__ inside the frozen archive.
        cls.Agent = load_agent(EXPERIMENT_SOURCE)
    ...
    def test_strong_evidence_stops_with_unexplored_affordable_hypotheses(self):
        env, agent = ExplorationEnv(), self.Agent()
    ...
    def test_search_exhaustion_and_timeout(self):
        ...
        with patch.object(self.Agent.act.__globals__["time"], "monotonic", side_effect=[0., 481.]):
            agent.act(ExplorationEnv())
        self.assertEqual("timeout", [e["reason"] for e in agent.log if e["event"] == "pilot_stop"][-1])


class MainKnownLimitTests(unittest.TestCase):
    """A passing characterization records a limitation, not a continuation fix."""

    def test_restored_main_stops_after_six_negative_pilots(self):
        env = ExplorationEnv(positive_cells=("x6", "x7"))
        MainAgent().act(env)
        self.assertEqual([-.4] * 6, [p["observed_lift_ratio"] for p in env.pilot_history])
        self.assertEqual(6, len(env.calls))
        self.assertFalse({"x6", "x7"} & {c["filter_current_tariff"] for c in env.calls})
        self.assertEqual(14, env.pilots_left)
        self.assertEqual(14400, env.remaining_contacts)
        self.assertEqual(100000, env.remaining_budget)
